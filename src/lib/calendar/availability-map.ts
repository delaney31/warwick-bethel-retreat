...
export type DayAvailabilityStatus = "available" | "pending" | "booked" | "blocked";
...
export function buildDayStatusMap(data: BookingCalendarData): Map<string, DayAvailabilityStatus> {
  ...
  for (const range of data.pendingRanges) {
    applyRange(map, range.start, range.end, "pending");
  }
  for (const range of data.bookedRanges) {
    applyRange(map, range.start, range.end, "booked");
  }
  for (const block of data.blocks) {
    applyRange(map, block.startDateUtc.slice(0, 10), block.endDateUtc.slice(0, 10), "blocked");
  }
  return map;
}
...
export function isDaySelectable(
  ...
  const status = getDayStatus(map, isoDate);
...
export function isStayRangeValid(
  ...
  for (const key of stayNightKeys(checkIn, checkOut)) {
    ...
    const status = getDayStatus(map, key);
    if (status === "booked" || status === "blocked") return false;
  }
  return true;
}
